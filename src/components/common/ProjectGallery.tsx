...
export default function ProjectGallery({ images, category, title, startDelay = 0 }: ProjectGalleryProps) {
  const autoplay = useRef(
    Autoplay({ delay: AUTOPLAY_INTERVAL, stopOnInteraction: false, stopOnMouseEnter: true, playOnInit: false })
  )
  ...
  useEffect(() => {
    if (!emblaApi) return
    const t = setTimeout(() => autoplay.current.play(), startDelay)
